...
const type = functions.config().database.type;
...
if (type === TYPE_FIRESTORE) {
  ...
  const { host, user, password, database } = functions.config().database;
  getConnection = async () => mysql.createConnection({ host, user, password, database });
} else {
